...
const ProductPage = () => {
  ...
  const {
    ...
    images = [], // Fallback to an empty array if undefined
  } = product;
    ...
    <div className="p-6 bg-white">
            ...
            {images.length ? (
              images.map((img, index) => (
                <div
                  ...
                  className="w-full h-150 bg-gray-200 rounded-xl overflow-hidden cursor-pointer"
                >
                  <img src={img} alt={`${name} ${index}`} className="object-cover w-full h-full" />
                </div>
              ))
            ) : (
      ...
      {selectedImage && (
        <div
          ...
          onClick={closeModal}
        >
          <img src={selectedImage} alt="Selected" className="w-full max-h-screen mb-10" />
        </div>
      )}
      <Relatedproduct currentProductId={id} />
    </div>
